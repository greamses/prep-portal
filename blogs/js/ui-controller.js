...
export function initUI(config, dataModule) {
  ...
  // Load published count from localStorage safely
  const saved = localStorage.getItem(`${config.collectionName}Count`);
  if (saved && publishCountSpan) {
    publishCount = parseInt(saved);
    publishCountSpan.innerText = publishCount;
  }

  // Register Auth Listener AFTER config is initialized
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      if (authStatusSpan) {
        authStatusSpan.innerHTML = `<span class="status-dot"></span>${escapeHtml(short)}`;
      }
      addLog(`[AUTH] ${user.email}`, "success");
      const ok = await loadApiKeys(user, subjectConfig);
      if (ok && !activeTimeout) {
        addLog(
          `[READY] Starting ${subjectConfig?.name} scheduler...`,
          "success",
        );
        scheduleNextRun(
          Math.floor(Math.random() * 5 * 60 * 1000 + 10 * 60 * 1000),
        );
      }
...
function addLog(msg, type = "info") {
  ...
  const cls =
    { success: "log-success", error: "log-error", warn: "log-warn" }[type] ||
    "log-info";
...
function scheduleNextRun(ms) {
  clearScheduler();
  if (!hasApiKeys()) {
    addLog("[SCHED] No API keys", "warn");
    return;
  }
  addLog(
    `[SCHED] Next ${subjectConfig?.name} post in ${Math.round(ms / 60000)} min`,
    "info",
  );
...
async function runPublishCycle() {
  await executePublishCycle(
    ...
    (modelLabel, provider, isFallback) => {
      ...
      addLog(
        `[MODEL] Using ${modelLabel}${isFallback ? " (fallback)" : ""}`,
        "info",
      );
    },
  );
...
function cleanDuplicateCssLinks(html) {
  ...
  const cssLink = `<link rel="stylesheet" href="/blogs/css/render.css">`;

  if (
    cleaned.includes('<div class="lesson-note">') ||
...
async function renderRecentPosts() {
  ...
  try {
    ...
    if (!posts.length) {
      list.innerHTML = `<li class="manage-empty">No ${subjectConfig?.name} posts yet.</li>`;
      return;
    }
    list.innerHTML = "";
    posts.forEach((post) => {
      const subj =
        post.subject || Object.keys(subjectLabels || {})[0] || "default";
      ...
      const hasVideos =
        post.videosAdded || (post.videos && post.videos.length > 0);
...
function attachButtonListeners(container) {
  ...
  container.querySelectorAll(".img-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      openImageEditor(btn.dataset.id, btn.dataset.title, btn.dataset.featured),
    );
  });
...
function openLinksModal(postId, postTitle, currentVideo, currentPractice) {
  ...
  if (subtitle)
    subtitle.textContent = `"${postTitle || "Untitled"}" — add video and interactive practice link.`;
  ...
  if (currentVideo && videoUrlInput)
    videoUrlInput.dispatchEvent(new Event("input"));
  if (currentPractice && practiceUrlInput)
    practiceUrlInput.dispatchEvent(new Event("input"));
...
async function openImageEditor(postId, postTitle, currentFeatured) {
  ...
  if (subtitle)
    subtitle.textContent = `"${postTitle || "Untitled"}" — paste image URLs per paragraph.`;
  if (paraBlocksList)
  ...
  try {
    const post = await getPost(postId);
    if (!post) {
      addLog("[IMG] Post not found", "error");
      return;
    }
    pendingImgContent = post.content || "";
    if (imgPendingBanner)
      imgPendingBanner.style.display =
        !post.featuredImage && !post.imagesAdded ? "flex" : "none";
...
function renderParaBlocks(html) {
  ...
  if (!blocks.length) {
    paraBlocksList.innerHTML =
      '<div class="manage-empty">No paragraph blocks found.</div>';
    return;
  }
  paraBlocksList.innerHTML = "";
  blocks.forEach((block, idx) => {
    const preview = block.textContent
      .trim()
      .replace(/\s+/g, " ")
      .substring(0, 90);
    const row = document.createElement("div");
...
if (saveContentBtn) {
  saveContentBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      addLog(
        `[CONTENT] Saved for ${getShortId(pendingContentId)}...`,
        "success",
      );
      if (contentModal) contentModal.classList.remove("active");
      pendingContentId = null;
...
if (saveVideosBtn) {
  saveVideosBtn.addEventListener("click", async () => {
    ...
    const videos = [];
    const rows = videoThumbnailsList
      ? videoThumbnailsList.querySelectorAll(".video-thumbnail-row")
      : [];
    ...
    rows.forEach((row) => {
      ...
      const duration =
        row.querySelector(".video-duration-input")?.value.trim() || "";
    ...
    try {
      ...
      addLog(
        `[VIDEOS] Saved ${videos.length} video(s) for ${getShortId(pendingVideosId)}...`,
        "success",
      );
      if (videosModal) videosModal.classList.remove("active");
...
if (saveImgBtn) {
  saveImgBtn.addEventListener("click", async () => {
    ...
    const hasAnyImg =
      !!featured || [...inputs].some((i) => i && i.value.trim());
    ...
    try {
      ...
      addLog(
        `[IMG] Images saved for ${getShortId(pendingImgId)}...`,
        "success",
      );
      if (imgModal) imgModal.classList.remove("active");
      pendingImgId = null;
...
if (saveMetaBtn) {
  saveMetaBtn.addEventListener("click", async () => {
    ...
    if (!title) {
      addLog("[META] Title required", "warn");
      return;
    }
    ...
    try {
      await updatePostMeta(pendingMetaId, {
        title,
        subject,
        classLevel: cls,
        excerpt,
  ...
  forceBtn.addEventListener("click", async () => {
    if (!hasApiKeys()) {
      if (currentUser) await loadApiKeys(currentUser, subjectConfig);
        ...
        addLog("[MAN] No user signed in", "error");
        return;
      }
    }
    clearScheduler();
...
if (testBtn) {
  testBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      addLog(
        `[OK] Found ${posts.length} ${subjectConfig?.name} posts`,
        "success",
      );
    } catch (e) {
      addLog(`[ERR] ${e.message}`, "error");
...
setInterval(() => {
  if (currentUser && hasApiKeys())
    console.log(`[ALIVE] ${subjectConfig?.name} publisher running`);
}, 30000);

addLog(
  `[READY] Publisher modules loaded — waiting for initialization...`,
  "info",
);
