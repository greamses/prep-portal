...
export function buildParentPanels(user, data, layout) {
  ...
  layout.innerHTML = `
    ...
    <div class="db-panel">
      <div class="db-panel-head">
        <div>
          ...
          <h2 class="db-panel-title">Practice Log</h2>
        </div>
      </div>
      <div class="db-feed">
        ${
          ...
          <div class="db-feed-item">
            <div class="db-feed-icon" style="background:#eee">${I.activity}</div>
            <div>
              <div class="db-feed-title">${a.title}</div>
              <div class="db-feed-sub">${fmtDate(a.date)} &bull; Accuracy: ${pct(a.score)}%</div>
            </div>
          </div>`,
                )
                .join("")
            : `<div class="db-empty">No recent activity.</div>`
        }
      </div>
    </div>
    ...
    <div class="db-panel">
      <div class="db-panel-head">
        <div><p class="db-kicker">Homework</p><h2 class="db-panel-title">Assigned Tasks</h2></div>
      </div>
      <div class="db-assign-list">
        ${tasks.length ? tasks.map((t) => assignmentItemHTML(t)).join("") : `<div class="db-empty">No pending assignments.</div>`}
      </div>
    </div>`;
