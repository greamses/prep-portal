/* ═══════════════════════════════════════════════════════
   ...
   Same algorithm as Drills'/Puzzles' js/matchmaking.js — only the room's
   config shape differs (shapes/given/lengths instead of operations/
   tables/fractionTypes) and it lives in its own geometryRooms/
   geometryRoomPointers collections. Two entry points:
   - matchmake(): anonymous pool matching for Multiplayer, via a single
     transaction on a small per-bucket pointer doc
     (geometryRoomPointers/{mode_size_timeLimit_shapes_given_lengths}), so
     two clients racing to start the same kind of room land in the SAME
     room instead of creating duplicates — Firestore's transaction retry
     handles the race, no manual conflict resolution needed.
...
function watchRoomUntilActive({ roomId, seed, roomSize, roomTimeLimit, roomShapes, roomGiven, roomLengths, isHost, waitMs, onWaiting }) {
  return new Promise((resolve, reject) => {
    const roomRef = doc(db, 'geometryRooms', roomId);
    let settled = false;
    let unsub = null;
    let hostTimer = null;
    ...
    function finish(data) {
      ...
      resolve({
        ...
        shapes: roomShapes, given: roomGiven, lengths: roomLengths,
...
async function joinOrCreateRoom({ mode, size, timeLimit, shapes, given, lengths, displayName: name }) {
  const user = auth.currentUser;
  const shapesKey = [...shapes].sort().join(',');
  const givenKey = [...given].sort().join(',');
  const lengthsKey = [...lengths].sort((a, b) => a - b).join(',');
  const bucketKey = `${mode}_${size}_${timeLimit}_${shapesKey}_${givenKey}_${lengthsKey}`;
  const ptrRef = doc(db, 'geometryRoomPointers', bucketKey);
  const now = Date.now();
  ...
  const result = await runTransaction(db, async (tx) => {
    ...
    if (ptrSnap.exists() && ptrSnap.data().expiresAt > now) {
      roomSnap = await tx.get(doc(db, 'geometryRooms', ptrSnap.data().roomId));
    }
    ...
    if (usable) {
      ...
      return {
        ...
        shapes: room.shapes, given: room.given, lengths: room.lengths,
      };
    }

    const roomRef = doc(collection(db, 'geometryRooms'));
    const seed = Math.floor(Math.random() * 2 ** 31);
    tx.set(roomRef, {
      mode, size, timeLimit, shapes, given, lengths, status: 'waiting',
    ...
    return { roomId: roomRef.id, isHost: true, size, timeLimit, seed, shapes, given, lengths };
  });
...
// Resolves once the room is "active": { roomId, seed, timeLimit, shapes, given, lengths, size, startAt, botsNeeded }.
// onWaiting(state) is called with { playerCount, size } while still in the lobby.
export async function matchmake({ mode, size, timeLimit, shapes, given, lengths }, { onWaiting } = {}) {
  const room = await joinOrCreateRoom({ mode, size, timeLimit, shapes, given, lengths });
  return watchRoomUntilActive({
    roomId: room.roomId, seed: room.seed, roomSize: room.size,
    roomTimeLimit: room.timeLimit, roomShapes: room.shapes, roomGiven: room.given, roomLengths: room.lengths,
...
export async function createCodeRoom({ mode = 'versus', size = 2, timeLimit, shapes, given, lengths, displayName: name }, { onWaiting } = {}) {
  const user = auth.currentUser;
  ...
  const roundReady = watchRoomUntilActive({
    ...
    roomShapes: shapes, roomGiven: given, roomLengths: lengths,
