/* ═══════════════════════════════════════════════════════
   VOCAB — matchmaking
   The same shape as the Drills page's matchmaking, against its own
   collections (vocabRooms / vocabRoomPointers) because a vocab room is
   described by subject+grade, not operations+tables — one rule block per
   game keeps each one's validation honest instead of loosening both.

   - matchmake(): anonymous pool matching, via a single transaction on a
     small per-bucket pointer doc, so two clients racing to start the same
     kind of room land in the SAME room instead of creating duplicates.
   - createCodeRoom()/joinRoomByCode(): private rooms joined with a short
     shared code. Longer wait window, since a human has to receive and type
     the code before a bot fills their seat.

   Both converge on watchRoomUntilActive(): one onSnapshot per client on its
   own room doc, torn down the instant status flips to "active".
═══════════════════════════════════════════════════════ */
...
async function joinOrCreateRoom({ mode, size, timeLimit, subject, grade, displayName: name }) {
  ...
  const result = await runTransaction(db, async (tx) => {
    ...
    if (ptrSnap.exists() && ptrSnap.data().expiresAt > now) {
      roomSnap = await tx.get(doc(db, 'vocabRooms', ptrSnap.data().roomId));
    }
